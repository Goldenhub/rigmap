...
import { ExternalLink, Tag, Bookmark, BookmarkCheck, Loader2 } from 'lucide-react';
...
export default function WorkspaceViewer({ workspaceId, imageUrl, devices, isSavedInitial = false }: WorkspaceViewerProps) {
  ...
  return (
    <div className="relative rounded-3xl overflow-hidden border border-white/80 bg-white/40 backdrop-blur-xl shadow-2xl shadow-indigo-500/10">
      <div className="relative flex justify-center bg-neutral-50/50">
        ...
        {devices.map((device) => (
          <div
            key={device.id}
            className="absolute w-10 h-10 -ml-5 -mt-5 flex items-center justify-center cursor-pointer z-10 group/hotspot"
            ...
            <AnimatePresence>
              {activeDevice === device.id && (
                <motion.div
                  ...
                  className="absolute bottom-full mb-5 left-1/2 -translate-x-1/2 w-72 bg-white/90 backdrop-blur-2xl border border-white/10 rounded-2xl p-5 shadow-2xl text-left z-50 ring-1 ring-black/5"
                >
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span className="bg-indigo-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest">
                      {device.price ? `$${device.price.toLocaleString()}` : 'Custom'}
                    </span>
                  </div>
                  {device.description && (
                    <p className="text-neutral-600 text-xs leading-relaxed font-medium">{device.description}</p>
                  )}
      ...
      <div className="absolute top-6 right-6 z-20">
        <button
          ...
          disabled={isSaving}
          className={`p-4 rounded-2xl backdrop-blur-xl border transition-all shadow-xl flex items-center gap-3 font-black uppercase tracking-widest text-[10px] ${
            isSaved 
              ? 'bg-indigo-600 text-white border-indigo-500 shadow-indigo-500/20' 
              : 'bg-white/80 text-neutral-900 border-white/50 hover:bg-white'
          }`}
        >
          {isSaving ? (
